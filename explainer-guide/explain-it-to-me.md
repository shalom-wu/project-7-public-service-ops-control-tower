...
Four pages: Executive Overview (KPI cards, weekly trend, top hotspots,
...
priority weights are **what-if selectors**, so a leadership meeting can push
"aging" up and watch the ranking re-sort live. The repo ships the real
`.pbix` file with all 1.07 million requests imported, plus its text-based
source (the semantic model and report definitions are generated by scripts,
so the whole dashboard is reviewable as code) and the full documentation
kit — data model, every DAX measure, and real Desktop screenshots.
